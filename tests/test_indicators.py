...
class TestIndicators(unittest.TestCase):
    ...
    def test_pivot_points_standard(self):
        ...
        # R1 > S1 (support is below resistance), ignoring the NaN first bar
        valid = result.dropna()
        self.assertTrue((valid["R1"] >= valid["S1"] - 1e-10).all())

    def test_pivot_points_use_previous_bar(self):
        """Levels must be derived from the prior bar, not the current one."""
        result = pivot_points(self.df, method="standard")
        # First bar has no prior bar to work from
        self.assertTrue(pd.isna(result["Pivot"].iloc[0]))
        expected = (self.df["High"].iloc[0]
                    + self.df["Low"].iloc[0]
                    + self.df["Close"].iloc[0]) / 3.0
        self.assertAlmostEqual(result["Pivot"].iloc[1], expected, places=9)
    ...
    def test_empty_dataframe_does_not_crash(self):
        ...
        for fn in (parabolic_sar, heikin_ashi, supertrend, adx, macd, obv, vwap, atr):
            self.assertEqual(len(fn(empty)), 0, f"{fn.__name__} on empty frame")

    def test_numpy_integer_period_accepted(self):
        self.assertEqual(len(sma(self.df, np.int64(20))), len(self.df))
        _validate_period(np.int32(5))

    def test_validate_period_rejects_bool(self):
        with self.assertRaises(ValueError):
